...
export const createPost3D = async (req, res) => {
  try {
    ...
    uploadImages(req, res, async (err) => {
      if (err instanceof MulterError || err) {
        ...
        const [design] = await Pool.query(
          'INSERT INTO designs (ID_SELLER, Name_product, Description, Model_Route, Screen_Model_Route, Type_Design) VALUES (?,?,?,?,?,?)',
          [sellerIs.seller, name, description, newPath, newScreen, Number(type)]
        );
  ...
  uploadImages(req, res, async (err) => {
    if (err instanceof MulterError || err) {
      ...
      const [sellerIs] = await Pool.query(
        'SELECT ID FROM seller WHERE ID_USER = (SELECT ID FROM users WHERE Email = ?)',
        [req.session.userEmail]
      );

      const [design] = await Pool.query(
        'INSERT INTO designs (ID_SELLER, Name_product, Description, Model_Route, Screen_Model_Route, Type_Design) VALUES (?,?,?,?,?,?)',
        [sellerIs[0].ID, name, description, '', '', Number(type)]
      );
      ...
      for (let i = 0; i < req.files.length; i++) {
        const [images] = await Pool.query(
          'INSERT INTO images (ID_PROJECT, Route) VALUES (?,?)',
          [designSeller[0].ID, req.files[i].path]
        );
...
export const showProfessions = async (req, res) => {
  try {
    const connection = await Pool.getConnection();
    const [result] = await connection.query('SELECT Type FROM catalog_designs');
    connection.release();
